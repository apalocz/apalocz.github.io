...
Cuts.rectangulate = function(trianglePart) {
    ...
    if (trianglePart.polygon.length !== 3) {
        throw "Attempting to canonify a non-triangular part";
    }
...
Cuts.stackRect = function(axisRect, newWidth) {
    ...
    for(let i = 0; i < numPieces; i++) {
        ...
        [right, left] = remainder.cut(cutLine);
    ...
    newRect = new AxisRect(axisRect.downleft, pieceWidth, axisRect.area);
...
Cuts.rectToRect = function(axisRect, newWidth) {
    targetRect = new AxisRect(axisRect.downleft, newWidth, axisRect.area);
...
Cuts.decompose = function(polygonA, polygonB) {
    ...
    if (Geometry.area(polygonB) - area > cutoff) {
        throw "areas are not the same: diff = "+Math.abs(Geometry.area(polygonB) - area);
    ...
    for(var partA of partsA) {
        for(var partB of partsB) {
            ...
            let newTransform = math.multiply(math.inv(partB.transform), partA.transform);
