...
import {FigureWithCaption} from  "../../components/commonComponents.jsx";
...
export const fig5Caption = "Comparison of Model A and Model B test outputs to target drawings";
export const fig8Caption = `A comparison of the line drawings produced by feeding the outputs of Model A to 
                    the ridge detection algorithm (top) and the sketch cleanup model (bottom)`;
export const fig9Caption = `Comparison of drawings by Artist A and Artist B with predictions of different 
                viewpoints of the same object for both subjects.`;
export const fig11Caption = (<>Visual comparison between the outputs from this process (top) and established 
...
a structure of down-sampling, followed by flat convolutions, followed by up-sampling.
...
While the project mostly focused on generating predictions based on aggregates of inputs from all 
...
suggesting that this could be a promising avenue for future study.
